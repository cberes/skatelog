...
- `ruff check` configuration came from [ruff's docs](https://docs.astral.sh/ruff/linter/#rule-selection) \["a configuration that enables some of the most popular rules (without being too pedantic)"]
- Some of the pre-commit configuration was generated by `pre-commit sample-config` (see [pre-commit's docs](https://pre-commit.com/#2-add-a-pre-commit-configuration))
